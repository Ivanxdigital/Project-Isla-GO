...
export default function DriverDashboard() {
  ...
  */
    ...
    showConfirmationModal(
      ...
      async (confirmed) => {
        ...
        try {
          ...
          console.log(`Notification response code: ${notification.response_code}`);
          
          // Validate response code
          if (!notification.response_code) {
            console.error('No response code found in notification');
            toast.dismiss(loadingToast);
          ...
          try {
            ...
            if (rpcResponse === 'SUCCESS') {
              ...
              toast.dismiss(loadingToast);
              toast.error('This booking request has expired');
            } else if (rpcResponse === 'BOOKING_NO_LONGER_AVAILABLE') {
              toast.dismiss(loadingToast);
  ...
  // Add a new helper function for the booking update fallback flow
  const useBookingUpdateFallback = async (bookingId, notificationId, driverId, accept, loadingToast) => {
    if (!accept) return; // Only implement fallback for acceptance
    
    try {
      console.log('Implementing fallback mechanism for booking acceptance');
      
      // 1. Update booking status directly
      const { error: bookingUpdateError } = await supabase
        .from('bookings')
        .update({
          status: 'DRIVER_ASSIGNED', // Use uppercase to match the status enum in database
      ...
      toast.dismiss(loadingToast);
      toast.error(`Error in fallback: ${error.message}`);
      return false;
    }
  };
